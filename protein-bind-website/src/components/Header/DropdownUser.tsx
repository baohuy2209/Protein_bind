    ...
    <ClickOutside onClick={() => setDropdownOpen(false)} className="relative">
      ...
      >
        <span className="hidden text-right lg:block">
          ...
          <span className="block text-xs text-black">{user.jobTitle}</span>
        </span>
        <span className="h-11 w-11 rounded-full text-black">
          <Image
            width={80}
            height={80}
            src={user.photo}
      ...
      {dropdownOpen && (
        <div className="absolute right-0 mt-4 flex w-62.5 flex-col rounded-lg border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
          <ul className="flex flex-col gap-5 border-b border-stroke px-6 py-7.5 dark:border-strokedark text-black">
            <li>
              <Link
                href="/profile"
