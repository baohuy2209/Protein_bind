...
import MoleculeStructureComponent from "../MoleculeStructure/index";
interface IMoleculeBankTableProps {}
const moleculeBank = [
  {
...
const MoleculeBankTable: React.FunctionComponent<
  IMoleculeBankTableProps
> = () => {
  const [searchQuery, setSearchQuery] = React.useState("");
  const [filterMolecules, setFilterMolecules] = React.useState(moleculeBank);
  React.useEffect(() => {
    const filteredData = moleculeBank.filter((molecule) => {
      molecule.moleculeName.toLowerCase().includes(searchQuery.toLowerCase());
    });
    setFilterMolecules(filteredData);
  }, [searchQuery]);
  return (
    <div className="rounded-lg border border-stroke bg-white px-5 pb-2.5 pt-6 shadow-default dark:border-[#181818] dark:bg-[#181818] sm:px-7.5 xl:pb-1">
      ...
      <input
        ...
        className=" text-black border-gray-300 text-gray-700 placeholder-gray-400 dark:border-gray-600 dark:border-gray-600 dark: placeholder-gray-500 text-md mb-4 w-full rounded-lg border bg-white px-4 py-3 shadow-sm outline-none focus:border-primary focus:ring-primary dark:bg-[#181818] dark:text-white-"
      />
      <div className="flex flex-col">
        <div className="grid grid-cols-3 rounded-lg bg-gray-2 dark:bg-[#121212] sm:grid-cols-4">
          ...
          <div className="hidden p-2.5 text-center sm:block xl:p-5">
            <h5 className=" text-black text-sm font-medium uppercase xsm:text-base">
              Category Usage
            </h5>
          </div>
          {filterMolecules.map((molecule, key) => (
            <div
              className={`grid grid-cols-3 sm:grid-cols-4 ${key === filterMolecules.length - 1 ? "" : "border-b border-stroke dark:border-strokedark"}`}
              key={key}
            >
